...
VERSION = 0.12.0
IMAGE_REPO ?= ghcr.io/gloryvu/frigate_dev
...
push: build
	docker buildx build --push --platform linux/amd64 --target=frigate --tag $(IMAGE_REPO):0.12.0 .
	docker buildx build --push --platform linux/amd64 --target=frigate-tensorrt --tag $(IMAGE_REPO):0.12.0-tensorrt .

run: local

	echo ${PWD}/config/config.yml
	docker run --rm --publish=5000:5000 --volume=/home/iec-ptit/Desktop/vvinh/frigate_dev/frigate-0.12.0/config:/config --volume=/home/iec-ptit/Desktop/vvinh/frigate_dev/frigate-0.12.0/debug:/media/frigate frigate:latest
